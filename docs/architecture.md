...
Initial context consists of merged declaration excerpts: approximately 50 source units before each declaration and at least 110 from its start, extended to include its name. The default usage mode then adds at most 2,048 UTF-8 prompt bytes per request from selected uses, subject to the original overall budget. Groups and relation facts are fixed before enrichment. Whole functions and whole switch cases are not added. At most the first 32 reference paths per binding are considered, with two candidates retained; ancestor searches stop at function boundaries or 12 levels. Short switch discriminants and case tests can accompany a use, linked by source offsets and marked for possible fallthrough. Overlapping use spans are merged and shared guards are deduplicated within a request. By default there is no cross-request memory. Optional linked propagation adds bounded prior-name hints after planning; see the [guide](guide.md#linked-request-propagation-experimental). Up to 64 in-group relation facts are retained, subject to the prompt budget. The byte measurement covers `promptFor(request)` in UTF-8, not the HTTP envelope or tokenizer overhead. Metadata records omitted facts and unexpanded reference counts. The provider output allowance is `128 + 64 × target count` tokens.
...
Concurrency limits in-flight calls; RPM spaces their start times. Responses may arrive out of order. Parseable answers retain valid target entries independently. Unknown IDs are ignored; unresolved IDs alone may receive one corrective attempt. Invalid JSON is not salvaged. Both attempts use the same pool and RPM budget. By default, names propagate only across the first-pass application boundary. With requestPropagation set to linked, a dependency-aware pool waits for selected earlier groups and supplies unverified name hints within the same pass. Independent ready groups still occupy available slots. Valid identifier spelling and binding safety are checked during final renaming.
