...
async function main(){
 ...
 try {
 const result=await recoverNames(code,{provider,passes,promptFormat,contextMode,requestPropagation,concurrency:Number(v.concurrency),rpm:Number(v.rpm),maxCalls:Number(v['max-calls']),signal:controller.signal,onEvent:async event=>{progress(event);await reporter?.onEvent(event);}});
