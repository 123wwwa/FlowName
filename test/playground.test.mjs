...
test('disconnect stops worker and streamed events do not leak supplied key',async()=>{
 ...
 const server=createPlayground({spawnWorker:(data)=>{assert.deepEqual(data.options,{passes:2,concurrency:8,rpm:120,maxCalls:50,promptBytes:8000,maxTargets:4});const w=new EventEmitter();w.terminate=()=>{terminated=true;};setTimeout(()=>w.emit('message',{type:'request',index:0,note:'fixture-secret'}),10);return w;}});
 await new Promise(r=>server.listen(0,'127.0.0.1',r));
 try{
  const response=await fetch(`http://127.0.0.1:${server.address().port}/api/recover`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'let a=1',apiKey:'fixture-secret',model:'m',provider:'gemini',options:{passes:2,concurrency:8,rpm:120,maxCalls:50,promptBytes:8000,maxTargets:4}})});
